...
- Commit `0a7d4ab` corrects the rule 11 check. The check of 11 is void. Filename starts `MEMORY` is 17, and that is the check. The fail mark is off the bar. The category name stays `memory-index`. The three live files are `MEMORY.md`, `MEMORY_B.md`, and `MEMORY_SIFT.md`. The other 14 stay review `bak`. SIFT doctrine is 7, including `FOREVER_SIFT_v1.md.bak-pre-s9-20260819`, which is still review `bak`. Raphael plans are the 10 named markdown files. Other is the 8 named files. Those checks match. `public/cli.json` and `public/web.json` were not edited. Note `web-1` stays.
